 ...
 * comprobación de esta pantalla que se puede hacer siempre. `portal.spec.mjs`
 * necesita un servidor y la API publicada, y `scripts/medir.mjs` necesita lo
 * publicado.
 *
 ...
 * conoce se queda visible encima de las demás cuando se cambia de pantalla. */
const vistas = [...html.matchAll(/id="(v-[a-z0-9-]+)"/g)].map((m) => m[1]);
const enMostrar = js.match(/for \(const v of \[([^\]]+)\]\) \$\(v\)\.hidden/)?.[1] ?? '';
const noConocidas = vistas.filter((v) => !enMostrar.includes(`'${v}'`));
 ...
 * es parte de ella. Recortarla aquí haría que una buena no entrara, y nadie
 * sabría por qué. */
